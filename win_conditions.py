...
def check_down(settings, board):
    ...
    for win_counter in range(1, settings.win_number + 1):
        ...
        if win_counter < settings.win_number and row < settings.rows and board[row][col]['player'] == settings.player:
            win_counter += 1
...
def check_right(settings, board):
    ...
    for win_counter in range(1, settings.win_number):
        ...
        if win_counter < settings.win_number and col < settings.cols and board[row][col]['player'] == settings.player:
            result += 1
...
def check_left(settings, board):
    ...
    for win_counter in range(1, settings.win_number):
        col -= 1
        if win_counter < settings.win_number and col > 0 and board[row][col]['player'] == settings.player:
            result += 1
        elif win_counter == settings.win_number:
            break
        else:
...
def check_diagonal_nw_se(settings, board):
...
def check_nw(settings, board):
    ...
    for win_counter in range(1, settings.win_number):
        ...
        if win_counter < settings.win_number and row > 0 and col > 0 \
                and board[row][col]['player'] == settings.player:
            result += 1
        elif win_counter == settings.win_number:
            break
        else:
...
def check_diagonal_ne_sw(settings, board):
